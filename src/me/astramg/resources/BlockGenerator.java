...
public class BlockGenerator {
	...
	public BlockGenerator(BlockLayer... layers) {
		...
		public void generateLayer(Location start, boolean effect) {
			...
			for (int i = 0; i < layer.length(); i++) {
				...
				if (character == ';') {
					...
					Block block = start.getWorld().getBlockAt(start.getBlockX() + i - skip, start.getBlockY(), start.getBlockZ() + rowZ);
