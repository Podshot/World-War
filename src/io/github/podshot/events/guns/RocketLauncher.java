		...
		if (gunIS.getType() == Material.MONSTER_EGG && gunIS.getDurability() == 50) {
			if (e.getPlayer().getLevel() >= 0) {
				...
				if (gun.equals("Rocket Launcher")) {
					CustomProjectile cpBullet = new ItemProjectile("bullet-rocket", e.getPlayer(), new ItemStack(Material.STONE), 2.5F);
					cpBullet.setIgnoreSomeBlocks(true);
					cpBullet.addTypedRunnable(new TypedRunnable<ItemProjectile>() {
						@Override
						public void run(ItemProjectile o) {
							Particles.FLAME.display(o.getEntity().getLocation(), 0, 0, 0, 0, 1);
						}
					});
					ItemProjectile bullet = (ItemProjectile) cpBullet.getEntity();
